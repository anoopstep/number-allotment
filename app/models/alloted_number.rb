class AllotedNumber < ApplicationRecord
  ...
  def check_availability_and_assign_number
    if self.number.present? && (AllotedNumber.pluck(:number).include? self.number)
      self.number = nil
    end
  ...
  def subscriber_number(alloted_numbers)
    ...
    if ((random_number.to_i > 1111111110) && (random_number.to_i < 10000000000) && !alloted_numbers.include?(random_number))
      random_number
    else
      nil
    end
